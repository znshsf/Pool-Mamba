...
class feature_pyramid_encoder(nn.Module):
    def __init__(self, channels):
        ...
        self.block0 = DualpoolingMamba(channels, scales=[12, 16, 20, 24], depth_p=2, depth_a=1) 

        self.down0 = Downsample(channels)

        self.block1 = DualpoolingMamba(channels * 2, scales=[6, 8, 10, 12], depth_p=2, depth_a=1) 

        self.down1 = Downsample(channels * 2)

        self.block2 = DualpoolingMamba(channels * 4, scales=[3, 4, 5, 6], depth_p=2, depth_a=1) 

        self.down2 = Downsample(channels * 4)

        self.block3 = DualpoolingMamba(channels * 8,  scales=[1, 2, 3, 4], depth_p=4, depth_a=2)
...
class feature_pyramid_decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.block3 = DualpoolingMamba(channels * 4, scales=[3, 4, 5, 6], depth_p=2, depth_a=1) 

        self.up2 = Upsample(channels * 4)

        self.block2 = DualpoolingMamba(channels * 2, scales=[6, 8, 10, 12], depth_p=2, depth_a=1)

        self.up1 = Upsample(channels * 2)

        self.block1 = DualpoolingMamba(channels, scales=[12, 16, 20, 24], depth_p=2, depth_a=1)  
